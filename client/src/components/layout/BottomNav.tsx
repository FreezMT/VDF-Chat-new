...
export function BottomNav({ unread }: { unread: number }) {
  return (
    <div className="pointer-events-none fixed inset-x-0 bottom-0 z-40 flex justify-center pb-[max(0.75rem,env(safe-area-inset-bottom))] pt-2">
      <nav
        className="pointer-events-auto flex w-[calc(100%-2rem)] max-w-xl items-stretch gap-0.5 rounded-full border border-white/10 bg-zinc-900/85 px-2 py-2 shadow-nav backdrop-blur-xl sm:max-w-2xl md:gap-1 md:px-3"
